...
탑다운 뷰가 "흔한 검은 QR" 로 바뀌지 않도록, 최종 화면을 **테마 색상의 스캔 카드**로
구성합니다. 위에서 내려다본 풍경 안에 QR 판이 놓인 구도입니다.

- 모듈은 검정이 아니라 **테마의 짙은 색**(예: B612 딥파인 `#2E4A38`),
  바탕은 흰색이 아니라 **테마의 밝은 색**(예: `#F1F6EC`)
- QR 판 아래에는 살짝 어긋난 **그림자 판**이 깔려 카드가 풍경 위에 얹힌 것처럼 보임
- 판 바깥에는 **테마의 풍경색**과 낮은 소품(풀포기 · 잔별 · 발자국 · 꽃잎)이 그대로 남음
- 조명을 끄지 않고 부드러운 톱라이트를 유지해 타일이 종이가 아니라 낮은 복셀로 읽힘

### 스캔 가능성 보장

그러면서도 스캔 성공률은 여러 겹으로 지킵니다.

1. **명암 대비 7.5:1 이상** — 네 테마 모두 WCAG AAA(7:1)를 넘는 대비로 색을 골랐습니다.
   (B612 8.9:1 · 별밤 12.4:1 · 사막 7.5:1 · 장미 9.5:1)
...
헤드리스 브라우저로 **4개 테마 × 모바일/데스크톱 뷰포트**의 탑다운 화면을 캡처해
QR 디코더로 되읽는 검증과, **저장되는 PNG 자체를 디코딩**하는 검증을 함께 돌립니다.
